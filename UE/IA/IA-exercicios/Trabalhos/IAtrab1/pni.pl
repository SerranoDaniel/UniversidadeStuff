...
pesquisa_it(Ln,Sol,P):- P1 is P+1,retractall(path(_)),pesquisa_it(Ln,Sol,P1).
...
escreve_seq_solucao(no(E,Pai,Op,Custo,Prof)):- 
	...
	write('Numero de nos visitados: '),
	nev(X), X1 is X-1,write(X1),
	nl,
	write('Numero maximo de nos em memoria: '),
	nmem(Y), write(Y),
	nl,
	write(custo(Custo)),
	nl,
