...
valor((E, _), 1, _):-(linhas(E);colunas(E);diagonais(E)),vencedor(o),!.
valor((E, _), -1, _):-(linhas(E);colunas(E);diagonais(E)),vencedor(x),!.
valor((E, _), 0, _):- empate(E),!.


% terminal se há uma linha coluna ou diagonal completa
terminal((E, _)):-
...
guarda(vencedor(P)):- retractall(vencedor(_)), asserta(vencedor(P)),!.

coluna(E,Y):-
	findall(P, (member((p(_,Y),P),E), atom(P)),L),
	length(L, S),
	S = 3,
	L = [O,O,O], guarda(vencedor(O)),!.
...
diagonais(E):-
	...
	diagonal_2(E, 1, 0, 0, NX2, _), NX2=3), guarda(vencedor(x)), !.

diagonais(E):-
	(diagonal_1(E, 1, 0, 0, _, NO1), NO1=3;
	diagonal_2(E, 1, 0, 0, _, NO2), NO2=3), guarda(vencedor(o)), !.
