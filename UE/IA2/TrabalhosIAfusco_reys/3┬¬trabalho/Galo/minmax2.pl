...
g_min(Jogo):- [Jogo], estado_inicial(Ei),
	iniciar_contagem,    %inicia a contagem dos nós
	statistics(real_time,[X,_]),  %tempo calculado por tempo final -- tempo inicial
	minimax_decidir(Ei,Op),
	statistics(real_time,[X1,_]), 
	T is X1-X,nl,
	write('Tempo em milesimos de segundo: '(T)),nl,
	n(Y),
	write('Numero de Nos: '(Y)),nl,
    write(Op),nl.

%inicializador do contador
iniciar_contagem:-
 retractall(n(_)),
    assertz(n(0)),!.
    
incrementa_contador(N):-
	retract(n(M)),
	M1 is N+M,
	assertz(n(M1)),!.
