...
n(0).
%incrementa  um nó
adicN(N) :- retract(n(X)), M is N+X, asserta(n(M)).


g(Jogo):- [Jogo], estado_inicial(Ei), minimax_decidir(Ei,Op),
     write('operador: '), write(Op),nl, n(X),  write('nos visitados: '),write(X), nl.
...
minimax_valor(Ei,Val,P):- terminal(Ei), adicN(1), valor(Ei,Val,P).
...
minimax_valor(Ei,Val,P):- adicN(1),
